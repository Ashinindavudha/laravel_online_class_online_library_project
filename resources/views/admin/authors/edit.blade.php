...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.authors.update", [$author->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="author_bio">{{ trans('cruds.author.fields.author_bio') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('author_bio') ? 'is-invalid' : '' }}" name="author_bio" id="author_bio">{!! old('author_bio', $author->author_bio) !!}</textarea>
                @if($errors->has('author_bio'))
                    <div class="invalid-feedback">
                        {{ $errors->first('author_bio') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.author.fields.author_bio_helper') }}</span>
            </div>
            <div class="form-group">
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '/admin/authors/ckmedia', true);
                ...
                data.append('crud_id', {{ $author->id ?? 0 }});
                xhr.send(data);
              });
            })
        }
